...
Process::Process(char* const* exec, char* const* envp, const char* program_name, tm_process_config &config, pid_t ppid, pid_t pgid): _program_name(program_name)
{
	this->pid = 0;
	this->ppid = ppid;
	this->pgid = pgid;
...
Process::_spawn(void)
{
	...
	if ((this->pid = spawn_child(this->exec, this->envp, this->std_in_fd, this->std_out_fd, this->std_err_fd, this->pgid)) == 0)
	{
